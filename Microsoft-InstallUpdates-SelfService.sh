...
# SCRIPT TYPE 
#
#	  SELF SERVICE - This will run on demand if the user wants to check for Microsoft updates.  The
#	  user WILL be informed if there are no updates available.
#
# SCRIPT VERSION
#
#	  1.0
...
Microsoft apps will be closed during the update.  Download and install these updates now?" \
  -button1 "INSTALL" -button2 "CANCEL" -defaultButton 0 -lockHUD)
	if [ "$userChoice" == "0" ]; then
    	...
    	"$jamfHelper" -windowType hud -heading "Microsoft Update Manager" \
    	-description "There are no updates available." \
    	-icon "$MAUicon" -button1 CLOSE -defaultButton 0 -lockHUD
