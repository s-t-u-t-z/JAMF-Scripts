...
# Download Individual Adobe Apps
...
function CleanUp ()
{
	...
	"$jamfHelper" -windowType hud -heading "Adobe Update Manager" \
    -description "All updates have been installed." \
    -icon "$ACCDicon" -button1 CLOSE -defaultButton 0 -lockHUD
...
secho=`sed 's/Following Updates are applicable on the system :/The following updates are ready to install:/g' $rumlog \
...
Adobe apps will be closed during the update.  Download and install these updates now?" \
